...
let parse_newclient_response (r: Result<string, string>) =
        match r with
        | Ok r -> (NewClientResponse.from r)
        | Error e -> Error e

let enroll_newclient (nc: Result<NewClientResponse, string>) (ctx: HttpContext) = task {
        ...
        | Ok nc ->
           ...
           let candidate = {
              ccode = (string nc.ccode);
              typ = "";
              id_or_name = "";
              comp_id = ""
              confidence = 0.98 //not using
           }
           let er = {
               command = "enroll"
           ...
           printfn "======== Enroll Candidate =========="
           printfn $"%A{er}"
           printfn "======== ===== =========="

           //STUDENTS CAN BE RETURNED BY CCODE but not SUSPECT for some reason.. Is it me?
           let! enroll_res = Helpers.enroll ctx er
           //get the search result.
           //convert to a generic data type.
           //get the image from the resulting url
            ...
            try
                ...
                match nc with
                 | Ok client ->
                        ...
                        let ncr = parse_newclient_response res
                        let! enroll_res = enroll_newclient ncr ctx
                        return! json enroll_res next ctx
                 | Error e  ->
